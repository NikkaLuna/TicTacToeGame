...
package ah.tictactoeapplication;
...
public class TicTacToe 
{
    ...
    {
        return number > 0 && number < board.length + 1; //must be between 1 and 9
    }
    
    //Check if the spot is taken
    public boolean isSpotTaken(int number) 
    {
        return board[number - 1] != '-'; //If the number does not equal '-' then it is taken.
    }
